...
def geometric_mean(values):
    return math.exp(sum(math.log(v) for v in values) / len(values)) if values else 0

def run_benchmark(exe, inputs, summary):
    check_system_settings()

    results = []
    exe_tmp = "/tmp/benchmark"
    shutil.copy(exe, exe_tmp)
    ...
    for input_file in inputs:
        ...
        time_cmd = ["time", exe_tmp]
        time_result = subprocess.run(time_cmd, stdin=open(input_file, "r"), stdout=subprocess.DEVNULL, stderr=subprocess.PIPE, text=True)
        memory_usage = extract_memory_usage(time_result.stderr)

        # Run perf benchmark
        perf_cmd = ["perf", "stat", "-d", exe_tmp]
        perf_result = subprocess.run(perf_cmd, stdin=open(input_file, "r"), stdout=subprocess.DEVNULL, stderr=subprocess.PIPE, text=True)
        task_clock = extract_task_clock(perf_result.stderr)

        results.append((test_name, task_clock, memory_usage, perf_result.stderr, time_result.stderr))
    ...
    summary_lines = [
        ...
        f"| Geometric Mean                                     | {geometric_mean(times):>9,.2f} | {round(geometric_mean(memories)):>11,} |",
    ]
