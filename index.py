...
def main(root_dir):
    ...
    with open(output, "w", encoding="utf-8") as f:
        ...
        f.write(generate_links("benchmark", benchmark_dir, coverage_dir, strip_ext=".txt", strip_prefix="benchmark"))
        f.write("</div>\n")

        f.write("<div style='width: 35%; padding: 10px;'>\n")
        f.write(generate_links("coverage", cover_dir, coverage_dir, strip_ext=".txt", strip_prefix="cover"))
        f.write("</div>\n")

        f.write("<div style='width: 35%; padding: 10px;'>\n")
        f.write(generate_links("bundled-coverage", cpack_dir, coverage_dir, strip_ext=".txt", strip_prefix="cpack"))
        f.write("</div>\n")
